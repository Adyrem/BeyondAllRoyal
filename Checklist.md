...
- [x] `MainMenu` scene (build index 0) with a title, an AI difficulty dropdown (Easy/Medium/Hard), a Singleplayer button, and a disabled Multiplayer placeholder button (`MainMenuController.cs`) — built from scratch (Camera, EventSystem with `InputSystemUIInputModule`, Canvas) by `BeyondAllRoyal → 2 - Setup Scenes` (`MainMenuSetup.CreateMainMenuScene`); still needs to actually be run + scene saved
