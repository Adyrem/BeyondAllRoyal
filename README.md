...
3. Open the main scene and run the one-time content setup from the Unity menu, in order:
   - `BeyondAllRoyal → 1 - Create ScriptableObjects`
   - `BeyondAllRoyal → 2 - Create Prefabs (run step 1 first)`
   - `BeyondAllRoyal → 3 - Import Sprites and Assign to Data + Prefabs`
...
Assets/Scripts/
  ...
  UI/         HUD, shop panel, health bars
  Editor/     ProjectSetup — generates ScriptableObjects/prefabs/sprites, run from the Unity menu
```
...
- `Assets/Scripts/Editor/ProjectSetup.cs` is the source of truth for how ScriptableObjects, prefabs, and sprites are generated/wired — re-run its menu items after pulling changes that touch unit/building data or sprites.
