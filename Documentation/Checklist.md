...
- [x] `ThemeSetup` now adds a background `Image` to `buildingInfoPanel`/`placementInfoPanel`/`productionPanel`/`endScreen`/the shop panel if one doesn't already exist, instead of silently skipping — a panel with only text/buttons and no backing image renders as text floating directly over the game world, which is hard to read
...
- [x] Fixed the minimum-reserve slider overlapping `buildingInfoPanel` in the same top-left corner: `CreateMinimumReserveSlider` now places it as the first sibling under the Canvas (later siblings draw on top, so it always renders behind every other HUD panel), and `HUD.OnBuildingSelected` disables it (`Slider.interactable = false`) whenever a building's info panel is showing, so it can't be dragged through the panel now covering it
